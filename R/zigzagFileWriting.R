zigzag$methods(
  ...
  initializeOutputFiles = function(prefix, post_pred_boolean = FALSE){
    ...
    if(post_pred_boolean){
      ...
      #   write.table(matrix(c("gen", gene_names), nrow = 1),
      #               file = paste0(output_directory, "/", prefix, "_Xg_lib_", i, ".post_predictive_output.log"),
      #               col.names = F, row.names = F, sep = "\t", quote = F)
      # }
  ...
  writeToYgVariancegOutputFiles = function(prefix, gen){
    ...
    write.table(matrix(c(gen, round(geneVariance_g, digits = 4)), nrow=1),
