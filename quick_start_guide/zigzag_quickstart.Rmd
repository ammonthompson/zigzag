...
### Introduction
Zigzag computes the posterior probability gene's are active or inactive given a set of RNA-seq libraries. Specifically, the data are relative expression estimates (*e.g.* TPM or FPKM) for a set of genes in a set of biological replicates. The inference model Zigzag implements is a hierarchical Bayesian mixture model. A typically sized dataset will take the MCMC about 10 hours on your average desktop computer, although this can vary quite a bit. Please see our preprint describing the model (in review: [preprint](https://www.biorxiv.org/content/10.1101/711630v2)).   
...
First, specify the finite mixture model by setting the number of mixture components. In our experience two active components are often necessary and sufficient to provide a good fit to the data. However, this may vary among samples and species. Zigzag assumes a single inactive distribution. 

A nieve approach to selecting the number of active subcomponents is to first assume a simple, one active component mixture model and use posterior predictive simulation to assess the adequacy of this model. If determined to be inadequate (real data is far from the predictive distribution) then specify a more complex model by adding more mixture components and reanalyzing. Repeat this until an adequate model is found or you nolonger see improvement in model fit. If you are unable to find a good enough fit, then the model assumed by zigzag may not be a good approximation of the processes that generated your data. 
...
A key challenge inherent to mixture model analysis, is the non-identifiability of mixture component parameters---means, variances and weights. This is often referred to as the label switching problem. When all mixture components have identical prior distributions, the marginal posterior distributions for component parameters are identical and multi-modal. This is because the likelihood is invariant to the index of the component parameters. This would result in every gene having a 0.5 probability of active expression. 

To mitigate this problem, identifiability constraints on parameters are necessary. We find a simple and effective way to impose identifiability constraints is to use truncated prior distributions that minimize the overlap of the support of each component mean. For each mean, an offset or threshold is set as a lower bound for active mixture components and the upper bound of the inactive mean prior distribution (see figure below).
...
When the MCMC completes, zigzag generates a number of mcmc summary files, plots, and diagnostics which are placed in a sub-directory within the mcmc directory called "mcmc_report".
...
```
Pdfs of trace plots of model parameters---except gene-specific parameters---are generated when the MCMC is complete and placed in the mcmc_report directory. 
...
Sensitivity analysis explores the sensitivity of the posterior distribution to prior specification by comparing posterior distributions to the prior distributions and to posterior distributions under different priors. 
...
Likewise, when assuming a prior threshold to the extreme left, all the posterior sample for the inactive mean is concentrated very close to the upper threshold (middle; blue historgram) indicating the data likelihood is pulling the posterior to values greater than the threshold at -3.
...
Pdf plots of posterior distributions relative to prior distributions are also automatically generated and placed in the mcmc_report directory when the MCMC is complete.
