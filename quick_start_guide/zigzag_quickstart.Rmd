...
```
This tutorial assumes the reader is familiar with the hierarchical mixture model that zigzag implements. If not, please see the original paper (in review: [preprint](https://www.biorxiv.org/content/10.1101/711630v2))
I recommend using Rstudio for this tutorial. A typically sized dataset will take the MCMC about 10 hours on your average desktop computer to get a large enough sample from the posterior distribution, although this can vary quite a bit.
...
We have provided example data for this tutorial. These are subsets of genes used in the zigzag publication. You can copy-and-paste the R code to run analyses. 
...
### Model assumptions
Zigzag assumes the data follows a finite mixture model consisting of one inactive distribution and one or more active distributions. Each mixture component follows a Normal distribution on a log-scale, i.e. symmetric, bell-shaped distributions with low-expressed genes displaying dropout from binomial sampling error.

Some data may significantly depart from the assumption of normality. For example, data that has been rounded to only a couple digits may significantly depart from a Normal distribution when log-transformed and lead to biased parameter estimates. Ways to check this are discussed below.

#### Specify mixture model
In our experience two active components are often necessary and sufficient. However, this may vary among samples and species. We assume a single inactive distribution because expression is more variable among replicate libraries and thus carry less information for more complicated distributions. We describe here different approaches to determining how many mixture components should be specified to make reliable posterior inferences. 

A nieve approach to selecting the number of active subcomponents is to first assume a simple, one active component mixture model and use posterior predictive simulation to assess the adequacy of this model. If determined to be inadequate, specify a more complex model by adding more mixture components and reanalyzing. Repeat this until an adequate model is found or you nolonger see improvement in model fit. 

To aid others as they interpret your results, include detailed reports of posterior sensitivity to prior assumptions and posterior predictive simulation (see sections below).  

#### Specify priors to impose identifiability constraints on component means
Another well known challenge inherent to mixture model analysis, is the non-identifiability of mixture component parameters---means, variances and weights. This is often referred to as the label switching problem. When all mixture components have identical prior distributions, the marginal posterior distributions for component parameters are identical and multi-modal. This is because the likelihood is invariant to the index of the component parameters. This would result in every gene having a 0.5 probability of active expression. 

To mitigate this problem, identifiability constraints on parameters are necessary. We find an effective way to impose identifiability constraints is to use truncated prior distributions that minimizes the overlap of the support of each mean. For each mean, an offset or threshold is set where the probability the mean is beyond the threshold is zero. In the case of the inactive mean, the threshold delimits the upper bound of its prior distribution.

To aid in setting these thresholds, you can inspect the empirical distribution of the data to specify component mean prior distributions. It is usually readily apparent where the modes of the latent mixture components are approximately located. By selecting prior thresholds between those modes you can impose identifiability on on the model. This approach is called Empirical Bayes because the data is used to specify the prior distribution. 

For example, in the plot below, the thick blue line on the left doubles as the upper boundary for the inactive mean prior distribution and the lower boundary for the first active mean prior distribution. The thin vertical red line on the far right shows the chosen lower boundary for a possible cluster of outlier genes with very high expression in the right tail. This is sometimes necessary because Normal distributions can be sensitive to outliers. The dashed lines show the default prior densities (scaled for visualization) for component means. 

Zigzag, by default, uses kernel density estimation to automatically select a model with one or two active components and set prior offsets or thresholds for all component means. In our experience, the range of the posterior distribution of the inactive mean is consistently below 1 log TPM and that of the active mean(s) is(are) consistently greater than -1 log TPM. This is not necessarily always going to be true for all data sets. We find this works well as a starting point, but often a visual inspection of kernel density fits using basic R functions, such as density, can allow users to set less informative prior thresholds that yield more robust posterior inferences. 

A fully Bayesian analysis would entail setting informative priors based on knowledge about the number of components and intervals that contain the inactive and active mixture component parameters. We expect such prior knowledge will rarely be detailed enough to be useful and that an empirical Bayesian approach with model refinement through model checking and sensitivity analysis would be an adequate approach.
...
### Load data and set hyperparameters in a zigzag object
...
Load the expression data and gene length data into a zigzag object. Specify a mixture model that contains two subdistributions in the active expression component with \textit{lower} boundaries set at 1 and 4 by setting threshold_a = c(1, 4). The \textit{upper} boundary for the inactive mean prior is by default equal to the threshold_a[1], in this case, 1. For all other priors we will keep the default settings. If num_active_components is not set, or set to "auto", zigzag will use kernel density estimation to determine whether one or two active subcomponents are detected in the data. 
...
Informative priors are often chosen out of practical necessity. It is important to choose priors that have a weak influence over the posterior distribution in these situations. Sensitivity analysis can reveal the influence of the prior on the posterior distribution. Sensitivity analysis explores the sensitivity of the posterior distribution to prior specification by comparing posterior distributions to the prior distributions and to posterior distributions under different priors. 

The plots below show an example of an analysis exploring posterior sensitivity to the choice of component mean prior thresholds. For this tutorial, several different analyeses of the example dataset were done with different thresholds separating the prior distributions for the inactive mean and the first active component mean (blue vertical line; below).

Thresholds targeting the region between the left shoulder and the central mode, ranging from -1 to 2 produce posterior distributions that are virtually identical and have little to no density at the prior threshold.
...
When the prior threshold is set to a value above the observed posterior for the active component---around 3 log TPM---the posterior becomes very sensitive to the prior.  Below the prior threshold was set at a high value to the right of the distribution mode. Under this prior, the posterior distribution for the active mean is highly concentrated at the threshold (right; red histogram).
...
Likewise, when assuming a prior threshold to the left of the inactive mean posterior distribution all the posterior sample for the inactive mean is concentrated very close to the upper threshold for the inactive distribution (middle; blue historgram) indicating the data likelihood is pulling the posterior to values greater than the threshold at -3.
...
For this dataset, any threshold between -0.5 log TPM and +2.5 log TPM yields the same posterior distribution because the data strongly suggest the inactive mean and first active mean lie on opposite ends of this interval (shaded in gray below). Any threshold placed within the gray interval will produce robust posterior inferences that are distant from the threshold. Zigzag's automatic threshold finding option often does a good job placing thresholds in this region when a new zigzag object is created.
...
Posterior predictive distributions of discrepancy statistics can be used to compare models with different prior thresholds for means and variance parameters and different numbers of active components. For example, posterior predictive simulations drawn during MCMC's for the prior threshold sensitivity analysis above show that the extreme thresholds that strongly influence the posterior distributions, also decrease the model fit to the data: the posterior predicitve distributions of the discrepency statistics tend to diverge from zero the more the proir thresholds move to the extreme tails of the empirical distributions.
...
Similarly, posterior predictive simulation can be used to assess the relative fit of models with different numbers of active mixture components. If we compare the posterior predictive distributions generated from a one component and two component posterior distribution, we see that the two component predictive interval is more centered on zero. This suggests a two-component model provides an improvement in fit for the example dataset:
...
This is found in the file: example_lung_probability_active.tab. If more than two active components are specified in the model, then the file example_lung_probability_in_component.tab gives the probability each gene is in each of component in the model.
